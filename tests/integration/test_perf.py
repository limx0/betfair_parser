...
def test_market_update_performance(benchmark):
    ...
    result: MCM = benchmark.pedantic(parse, args=(line,), rounds=1000, iterations=1000)
...
def test_market_definition_performance(benchmark):
    ...
    result: MCM = benchmark.pedantic(parse, args=(line,), rounds=10, iterations=10)
    assert isinstance(result, MCM)
    assert result.mc[0].marketDefinition
